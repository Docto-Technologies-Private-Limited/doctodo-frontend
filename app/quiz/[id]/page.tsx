...
import { X, ClipboardList, CheckCircle, AlertCircle, ChevronLeft } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className={`px-5 py-4 ${showBadge ? 'bg-red-50/30' : ''}`}>
        ...
        {/* Mobile: "Not Answered" badge above question text */}
        {showBadge && (
...
export default function QuizOverviewPage() {
  ...
  // All questions single-select: clicking same option deselects it
  const handleAnswer = useCallback((qId: string, value: string) => {
    setAnswers((prev) => {
      const current = prev[qId] ?? [];
      return {
        ...prev,
        [qId]: current.includes(value) ? [] : [value],
      };
    });
  }, []);
  ...
  const handleSubmit = () => {
    ...
    if (unanswered.length > 0) {
      setShowErrors(true);
      document.getElementById(`q-block-${unanswered[0].id}`)?.scrollIntoView({
        behavior: 'smooth',
        block: 'center',
      });
      return;
    ...
    <div className="flex min-h-screen bg-[#f4f5f7]">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <TopBar
          ...
          activeKey="quiz"
          userName="David Joe"
          userEmail="Davidjoe30@gmail.com"
          notificationCount={2}
        />
        ...
        <main className="flex-1 overflow-y-auto mt-[80px] mb-[60px] sm:mb-0 p-4 sm:p-6 lg:p-8">
          <div className="max-w-3xl mx-auto flex flex-col gap-4">
            ...
            {/* Quiz Header Card
                Mobile: title full-width, "5 Questions" pill below subtitle
                Desktop: title left, pill right (flex row) */}
            ...
            <div className="flex justify-center py-2 pb-6">
              <button
                ...
                className="bg-primary hover:brightness-90 active:brightness-75 text-white font-semibold px-10 py-3 rounded-xl flex items-center gap-2 text-sm transition-all shadow-sm w-full sm:w-auto justify-center"
              >
