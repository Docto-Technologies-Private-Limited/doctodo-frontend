    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col gap-3">
      ...
      <div className="pl-5">
        {quiz.status === 'completed' ? (
          <Link href={`/quiz/${quiz.id}/results`}
            className="flex items-center justify-center gap-1.5 bg-success text-white text-sm font-semibold px-4 py-2.5 rounded-lg w-full">
            View Results
          </Link>
        ) : (
          <Link href={`/quiz/${quiz.id}`}
            className="flex items-center justify-center gap-1.5 bg-primary text-white text-sm font-semibold px-4 py-2.5 rounded-lg w-full">
            Start Quiz →
          </Link>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-2 pt-4 border-t border-gray-100">
      <div className="flex items-center gap-3 text-xs text-gray-500">
        <span>Showing <span className="font-semibold text-gray-700">{start}–{end}</span> of{' '}
          <span className="font-semibold text-gray-700">{totalItems}</span></span>
      ...
      <div className="flex items-center gap-1">
        <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}
          className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed text-xs">‹</button>
        {pages.map((p, i) =>
          p === '...' ? (
            ...
            <button key={p} onClick={() => onPageChange(p as number)}
              className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs font-medium border ${
                currentPage === p ? 'bg-primary text-white border-primary' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
              {p}
            </button>
          )
        )}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="relative w-full rounded-2xl overflow-hidden" style={{ minHeight: 160 }}>
        <Image src="/images/banner/quiz_banner.webp" alt="Quiz Banner" fill className="object-cover" priority />
      </div>
        ...
        <div className="px-5 sm:px-6 pt-5 pb-4 border-b border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-4">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search by quiz name..." value={search}
                ...
                .map(({ value, label }) => (
                  <button key={value} onClick={() => handleStatus(value)}
                    className={`px-3 py-2 rounded-lg text-xs font-semibold border transition-all ${
                      statusFilter === value ? 'bg-secondary text-white border-secondary' : 'bg-white text-gray-600 border-gray-200 hover:border-secondary hover:text-secondary'}`}>
                    {label}
                  </button>
                ))}
            </div>
              ...
              <tbody>
                {paginated.map((quiz, index) => (
                  <tr key={quiz.id} className="border-t border-gray-100 hover:bg-gray-50/60 transition-colors">
                    <td className="px-6 py-4 text-gray-400 text-xs">{(currentPage - 1) * pageSize + index + 1}</td>
        ...
        {filtered.length > 0 && (
          <div className="px-5 sm:px-6 pb-5">
            <Pagination currentPage={currentPage} totalPages={totalPages} pageSize={pageSize}
              totalItems={filtered.length} onPageChange={setCurrentPage} onPageSizeChange={handlePageSize} />
          </div>
        )}
      </div>
    </div>
